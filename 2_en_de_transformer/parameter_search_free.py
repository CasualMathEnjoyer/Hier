...
OPTUNA_CROSS_STUDY_CACHE = "main_30_cache_free.json"
...
def objective(trial):
    ...
    # Free parameter search with integer boundaries
    model_settings = {
        "h": trial.suggest_int("h", 1, 4),  # number of heads
        "d_k": trial.suggest_int("d_k", 16, 48),  # key dimension
        "d_ff": trial.suggest_int("d_ff", 256, 1536),  # feedforward layer size
        "d_model": trial.suggest_int("d_model", 128, 300),  # model dimension
        "n": trial.suggest_int("n", 1, 3)  # number of layers
    }
...
if __name__ == "__main__":
    ...
    for sampler in samplers.keys():
        ...
        study = optuna.create_study(study_name=f"bakalarka_{sampler_name}_free_exploration_local",
